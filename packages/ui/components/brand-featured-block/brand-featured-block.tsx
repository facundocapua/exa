...
export const BrandFeaturedBlock = async ({ handle }: Props) => {
  ...
  return (
    <div className={clsx(
      ...
      { 'md:grid-cols-[33%_auto]': brand.featured_banner }
    )}>
      {brand.featured_banner && (<Link href={`/brand/${brand.handle}`}><Image src={brand.featured_banner} alt={brand.name} width={620} height={620} /></Link>)}
      <div className='flex justify-evenly gap-2 [&>*:nth-child(n+3)]:hidden md:[&>*:nth-child(n+3)]:flex'>
        {products.map((product) => (
          <ProductCardV2 key={product.id} product={product} containerClassName={clsx(
            'flex-shrink-0 flex-grow-0',
            { 'basis-1/2 md:basis-1/3': brand.featured_banner },
