...
export default function OrderTotals ({ order }: Props) {
  const shippingPrice = Number(order.shipping_total) / 100
  return (
    <>
      <Divider />
      <div className='flex flex-col gap-y-2'>
        ...
        <div className="flex justify-between gap-x-6 ">
          ...
          <div className="shrink-0 flex flex-col items-end">
            <p className="text-base leading-6 text-neutral-800 dark:text-neutral-200">{shippingPrice >= 1 ? formatPrice(shippingPrice) : 'Gratis'}</p>
          </div>
        </div>
        {
